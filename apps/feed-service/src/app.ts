...
(async function ApolloServerInit() {
  ...
  const server = new ApolloServer<FeedServiceContext>({
    schema: Modules.schemas,
    csrfPrevention: true,
    plugins: [
      responseCachePlugin<FeedServiceContext>(),
  ...
  app.get("/feed-service/health", (req, res) => {
    res.send({ status: "ok" });
  });
  app.use(
    "/feed-service",
  ...
  httpServer.listen(port, () => {
    console.log(
      "\x1b[33m",
      `Server ready at http://localhost:${port}/feed-service`,
      "\x1b[0m"
    );
  });
})();
