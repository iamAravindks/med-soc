...
export const getPostByIdOperation = async (
  postId: string,
...
export const getAllPostOperation = async (
  postId: string,
  server: TestApolloServer
) => {
  const result = await server.apollo.executeOperation(
    getAllPost("", { _id: { $eq: postId } }, { createdAt: 1 }, 10, 0),
    {
      contextValue: MockContextValue(
        ContextValueType.mhToken,
        server.redisClient
...
export const getOnePostOperation = async (
  postId: string,
  server: TestApolloServer
) => {
  const result = await server.apollo.executeOperation(
    getOnePost({ _id: { $eq: postId } }, { createdAt: 1 }),
    {
      contextValue: MockContextValue(
        ContextValueType.mhToken,
        server.redisClient
...
export const createPostOperation = async (
...
export const createManyPostOperation = async (
  datas: CreatePostInput[],
  server: TestApolloServer
) => {
  ...
  const refinedResult: any = get(
    result,
    "body.singleResult.data.createManyPost"
  );
  const refinedError = get(result, "body.singleResult.errors");
...
export const updatePostOperation = async (
...
export const updateManyPostOperation = async (
  datas: UpdatePostInput[],
  server: TestApolloServer
) => {
...
export const deletePostOperation = async (
  postId: string,
  server: TestApolloServer
) => {
...
export const deleteManyPostOperation = async (
  postId: string,
  server: TestApolloServer
) => {
  const result = await server.apollo.executeOperation(
    deleteManyPost({ _id: { $eq: postId } }),
    {
      contextValue: MockContextValue(
        ContextValueType.mhToken,
        server.redisClient
